...
import { Shield, X, Timer, Loader2 } from 'lucide-react';
 ...
 * Shows "Processing..." when countdown reaches 0 and awaits reroute_executed.
 */
const CountdownBar = memo(function CountdownBar({ shipmentId }) {
  const countdown = useCountdownStore((s) => s.countdowns[shipmentId]);
  const tickCountdown = useCountdownStore((s) => s.tickCountdown);
  ...
  }, [shipmentId, countdown?.status, tickCountdown]);

  if (!countdown) return null;

  const { seconds_remaining, seconds_total, shipment_name, status } = countdown;
  const isProcessing = status === 'processing';
  const progress = isProcessing ? 100 : ((seconds_total - seconds_remaining) / seconds_total) * 100;
  const isUrgent = seconds_remaining <= 30 || isProcessing;
      ...
      >
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold text-theme-primary">
                  {isProcessing ? 'Executing Auto-Reroute...' : 'Auto-Reroute Countdown'}
                </h4>
                <p className="text-[11px] text-theme-secondary">
                  {isProcessing 
                    ? `${shipment_name || `Shipment ${shipmentId.slice(-6)}`} — calculating optimal route`
            ...
            <div className="flex items-center gap-3">
              ...
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-mono text-sm font-black ${
                ...
                {isProcessing ? '...' : formatTime(seconds_remaining)}
              </div>
          ...
          <div className="h-1.5 w-full bg-theme-tertiary rounded-full overflow-hidden">
            <motion.div
              className={`h-full rounded-full ${isProcessing ? 'bg-red-500 animate-pulse' : isUrgent ? 'bg-red-500' : 'bg-amber-500'}`}
              initial={{ width: 0 }}
